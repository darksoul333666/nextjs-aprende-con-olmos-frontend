...
export const StudentStats: React.FC<StudentStatsProps> = ({ availableCourses }) => {
  const totalStudents = availableCourses.reduce((total, course) => total + (course.totalStudents || 0), 0);
  const totalHours = Math.round(availableCourses.reduce((total, course) => total + (course.totalDuration || 0), 0) / 3600);
